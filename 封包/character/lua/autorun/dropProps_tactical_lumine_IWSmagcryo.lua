local enable_pm_bodygroup_drop = CreateConVar("yl_pm_tactical_lumine_IWSmagcryo_bgd", "0", {FCVAR_ARCHIVE}, "PM模型掉落") -- 是否开启PM模型掉落身体部件
local enable_npc_bodygroup_drop = CreateConVar("yl_npc_tactical_lumine_IWSmagcryo_bgd", "0", {FCVAR_ARCHIVE}, "NPC模型掉落") -- 是否开启NPC模型掉落身体部件

local targetBodygroupName = "IWSmagcryo"
...
local dropModel = "models/elemental_warfare/tactical_lumine/props/IWSmagcryo.mdl"
-- 同步Bodygroup状态到所有客户端
local function BpStateSync(ent, bgID)
    if IsValid(ent) then
        net.Start("BpStateSync_tactical_lumine_IWSmagcryo")
            net.WriteEntity(ent)
...
local function ObjCreatedSync(models, pos, ang)
    net.Start("ObjCreatedSync_tactical_lumine_IWSmagcryo")
...
local function createDrop(drop, models, pos, ang)    
    ...
    if IsValid(drop) then
        ...
        if IsValid(phys) then
            phys:ApplyForceCenter(Vector(0.5*phy_num, 0.5*phy_num, 1) * 100)
        else
...
if SERVER then
    util.AddNetworkString("BpStateSync_tactical_lumine_IWSmagcryo")
    util.AddNetworkString("ObjCreatedSync_tactical_lumine_IWSmagcryo")

    -- 注册实体死亡事件
    hook.Add("PlayerDeath", "DropBodyParts_tactical_lumine_IWSmagcryo", function(ply)
    ...
    -- 注册NPC死亡事件
    hook.Add("OnNPCKilled", "DropNPCBodyParts_tactical_lumine_IWSmagcryo", function(npc)
...
if CLIENT then
    net.Receive("BpStateSync_tactical_lumine_IWSmagcryo", function()
    ...
    net.Receive("ObjCreatedSync_tactical_lumine_IWSmagcryo", function()
